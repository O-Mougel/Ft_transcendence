...
window.rejectFriend = async (friendId) => {
    ...
    if (!requestList || !requestLabel || !requestBlock || !friendId)
        return;
    const data = {
...
const checkForFriendRequests = async () => {
    ...
    const requestLabel = document.getElementById('requestCheckLabel');
    const requestBlock = document.getElementById('pendingRequestBlock');
    if (!requestList || !requestLabel || !requestBlock)
        return;
    try {
...
window.sendNewFriendRequest = async function () {
    ...
    if (!friendReqInput || !friendReqInput.value)
        return;
    const data = {
...
window.loadPlayer2Data = async function () {
    ...
    if (!Player2Name || !player2Pfp)
        return;
    try {
        const dataRequestResponse = await fetch('/profile/grab2', {
...
window.handlePongModeDisplay = async function (mode) {
    ...
    if (!LeftPlayer || !RightPlayer || !instruction1v1 || !instruction2v2)
        return;
    try {
        const dataRequestResponse = await fetch('/profile/grab', {
            headers: { Authorization: `Bearer ${sessionStorage.getItem("access_token")}` },
