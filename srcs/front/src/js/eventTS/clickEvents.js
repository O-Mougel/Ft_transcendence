...
window.onFileSelected = function (inputFileSelector) {
    ...
    if (!inputFileSelector || !selectedFileName)
        return;
...
window.validatePlayerNameFields = function (nbPlayers, event) {
    for (let i = 1; i <= nbPlayers; i++) {
        const currentInput = document.getElementById(`player${i}`);
        if (currentInput) {
...
window.createCustomTournamentPage = function (nbPlayers) {
    ...
    if (!tournamentBuiltBlock || !tournamentNbPlayerSelect)
        return;
...
window.saveProfileInfo = async function () {
    ...
    const selectedFileName = document.getElementById('selectedFileName');
    if (!username || !confirmText || !fileInput || !userPfp || !selectedFileName)
        return;
