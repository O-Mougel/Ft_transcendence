...
window.fillFriendRemovalBox = async (friendArray) => {
    const friendRemover2000 = document.getElementById("selectBoxFriendRemover");
    if (!friendRemover2000 || !friendArray)
        return;
...
window.grabLoggedUserStats = async () => {
    ...
    if (!nbOfMatchCpt || !winRatioPercent || !longestMatchCpt || !biggestStreakCpt)
        return;
...
window.fetchPlayerStats = async (playerId, playerUsername) => {
    ...
    if (!playerId || !nbOfMatchCpt2 || !winRatioPercent2 || !longestMatchCpt2 || !biggestStreakCpt2 || !friendStatDisplayBox || !selectedPlayerUsernameHeader)
        return;
...
export const show2FAStatus = async () => {
    ...
    if (!TwoFADisabled || !TwoFAActivated || !qrCodeImage || !TwoFACodeInput || !showQRCodeButton)
        return;
    try {
...
const createFriendsStatLink = async () => {
    ...
    if (!friendlistProfileParent || !currentUser)
        return;
    try {
...
const grabUserStatsAndInfo = async () => {
    ...
    const selectedPlayerUsernameHeader = document.getElementById('selectedPlayerUsernameHeader');
    const friendStatDisplayBox = document.getElementById('friendStatDisplayBox');
    if (!playerUsernameProfile || !userPfpProfile || !selectedPlayerUsernameHeader || !friendStatDisplayBox)
        return;
...
export const adjustNavbar = async (path) => {
    ...
    if (path === "/customizeProfile") {
        ...
        const profileBtn2 = document.getElementById('profileButton2');
        const profileBtn = document.getElementById('profileButton');
        if (profileBtn2)
            profileBtn2.style.display = 'flex';
        if (profileBtn)
