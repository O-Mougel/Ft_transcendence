...
function collectPlayerNames(expectedCount) {
    ...
    for (let i = 1; i <= expectedCount; i++) {
        ...
        if (!element)
            throw new Error(`Missing input player${i}`);
...
export async function startTournament(expectedCount, event) {
    ...
    socket.once("tournament:state", (data) => {
    ...
    socket.once("tournament:error", (data) => {
        const e = data;
