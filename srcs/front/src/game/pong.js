import { waitStartGame, isSocketConnected, setupSocket, joinExistingGame, emitNextMatch } from "./socket.js";
...
export function initPong(mode = {}) {
    ...
    if (CONTEXT.tournamentId /* && CONTEXT.gameId*/) {
        ...
        if (CONTEXT.startButton)
            CONTEXT.startButton.style.display = "none";
        if (CONTEXT.score)
            CONTEXT.score.style.display = "flex";
        if (CONTEXT)
...
export function updateGameScene(data) {
    ...
    // Ball
    if (typeof data.ball?.x === "number")
        ball.x = data.ball.x * GAME_WIDTH;
    if (typeof data.ball?.y === "number")
        ball.y = data.ball.y * GAME_HEIGHT;
    if (typeof data.ball?.vx === "number")
        ball.speedX = data.ball.vx;
    if (typeof data.ball?.vy === "number")
        ball.speedY = data.ball.vy;
    if (typeof data.paddles?.left === "number")
        leftPaddle.y = data.paddles.left * GAME_HEIGHT;
    if (typeof data.paddles?.right === "number")
        rightPaddle.y = data.paddles.right * GAME_HEIGHT;
    if (CONTEXT.gameMode === 2) {
        const { leftPaddle2, rightPaddle2 } = CONTEXT;
        if (typeof data.paddles?.left2 === "number")
            leftPaddle2.y = data.paddles.left2 * GAME_HEIGHT;
        if (typeof data.paddles?.right2 === "number")
            rightPaddle2.y = data.paddles.right2 * GAME_HEIGHT;
    }
    // Score
    if (data.score) {
